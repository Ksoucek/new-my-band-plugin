...
// New function: Add event to Google Calendar
function add_event_to_google_calendar($event_details) {
    $api_key = get_option('my_team_plugin_google_calendar_api_key'); // Fetch Google Calendar API key
    if (!$api_key) {
        error_log('API key for Google Calendar is not set.');
        return [
            'error' => 'API key for Google Calendar is not set.'
        ];
    }
    ...
    if (is_wp_error($response)) {
        error_log('Error communicating with Google Calendar API: ' . $response->get_error_message());
        return [
            'error' => 'Error communicating with Google Calendar API.',
...
function handle_add_to_calendar_request(WP_REST_Request $request) {
    ...
    error_log('Received event_details: ' . print_r($event_details, true));

    if (!$event_details) {
        error_log('Missing parameter: event_details');
    ...
    if (isset($result['error'])) {
        error_log('Error in add_event_to_google_calendar: ' . $result['error']);
